...
/// }
/// ```
pub trait Retryable<B: BackoffBuilder, Fut: Future<Output: BorrowError>, FutureFn: FnMut() -> Fut> {
    /// Generate a new retry
    fn retry(self, builder: B) -> Retry<B::Backoff, Fut, FutureFn>;
}

impl<B, Fut, FutureFn> Retryable<B, Fut, FutureFn> for FutureFn
where
    B: BackoffBuilder,
    Fut: Future<Output: BorrowError>,
    FutureFn: FnMut() -> Fut,
{
    fn retry(self, builder: B) -> Retry<B::Backoff, Fut, FutureFn> {
...
pub struct Retry<
    ...
    RF = fn(&<<Fut as Future>::Output as BorrowError>::Error) -> bool,
    NF = fn(&<<Fut as Future>::Output as BorrowError>::Error, Duration),
> {
    backoff: B,
    retryable: RF,
    notify: NF,
    ...
    state: State<Fut, SF::Sleep>,
}

impl<E, B, R, Fut, FutureFn> Retry<B, Fut, FutureFn>
where
    B: Backoff,
    Fut: Future<Output = R>,
    R: BorrowError<Error = E>,
    FutureFn: FnMut() -> Fut,
{
...
impl<B, E, R, Fut, FutureFn, SF, RF, NF> Retry<B, Fut, FutureFn, SF, RF, NF>
where
    B: Backoff,
    Fut: Future<Output = R>,
    R: BorrowError<Error = E>,
    ...
    NF: FnMut(&E, Duration),
{
    ...
    /// }
    /// ```
    pub fn sleep<SN: Sleeper>(self, sleep_fn: SN) -> Retry<B, Fut, FutureFn, SN, RF, NF> {
        Retry {
    ...
    /// }
    /// ```
    pub fn when<RN: FnMut(&E) -> bool>(self, retryable: RN) -> Retry<B, Fut, FutureFn, SF, RN, NF> {
        Retry {
            backoff: self.backoff,
            retryable,
            notify: self.notify,
...
#[derive(Default)]
enum State<Fut: Future, SleepFut: Future<Output = ()>> {
    #[default]
    Idle,
    Polling(Fut),
    Sleeping(SleepFut),
...
/// Trait representing borrowing the error from within the future result.
///
/// If [`as_error`](Self::as_error) returns `Some(error)`, returned error will be considered as a call failure
/// leading to a retry.
///
/// Used to abstract over various responses retryable functions can have. `Option<Result<T, E>>`, for instance, is useful with [stream unfolding](::futures::stream::unfold).
pub trait BorrowError {
    /// Type of the error of the retriable function. It is passed to [`notify`](Retry::notify) and [`when`](Retry::when).
    type Error;

    /// Borrow the error if it is present.
    fn as_error(&self) -> Option<&Self::Error>;
}

impl<T, E> BorrowError for Result<T, E> {
    type Error = E;
...
impl<T, E> BorrowError for Option<Result<T, E>> {
    type Error = E;

    #[inline]
    fn as_error(&self) -> Option<&Self::Error> {
        self.as_ref().and_then(|v| v.as_ref().err())
    }
}

impl<B, R, Fut, FutureFn, SF, RF, NF> Future for Retry<B, Fut, FutureFn, SF, RF, NF>
where
    B: Backoff,
    Fut: Future<Output = R>,
    R: BorrowError,
    FutureFn: FnMut() -> Fut,
    SF: Sleeper,
    RF: FnMut(&<R as BorrowError>::Error) -> bool,
    NF: FnMut(&<R as BorrowError>::Error, Duration),
{
    type Output = R;
        ...
        loop {
            match &mut this.state {
                ...
                State::Polling(fut) => {
                    ...
                    let value = ready!(fut.as_mut().poll(cx));

                    let Some(err) = value.as_error() else {
...
mod unfold_test {
    ...
    async fn wrap_less_than(
        ...
        if value < less_than {
            Some(if value % 2 == 0 {
                ...
                Err(anyhow!("sample test_query meets error"))
            })
        } else {
            None
        }
    ...
    #[test]
    async fn test_unfold_stream() {
        let mut unfolding_stream = stream::unfold(0, |mut state| {
            state += 1;

            (move || wrap_less_than(state, 3))
                .retry(ConstantBuilder::default().with_delay(Duration::from_secs(0)))
                .map(move |v| v.map(move |r| (r, state)))
        });

        let mut stream = pin!(unfolding_stream);
