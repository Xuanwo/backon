...
/// }
/// ```
pub trait Retryable<'a, B: BackoffBuilder, T, E, FutureFn: AsyncFnMut() -> Result<T, E>> {
    /// Generate a new retry
    fn retry(self, builder: B) -> Retry<'a, B::Backoff, T, E, FutureFn>;
}

impl<'a, B, T, E, FutureFn> Retryable<'a, B, T, E, FutureFn> for FutureFn
where
    B: BackoffBuilder,
    FutureFn: AsyncFnMut() -> Result<T, E>,
{
    fn retry(self, builder: B) -> Retry<'a, B::Backoff, T, E, FutureFn> {
...
pub struct Retry<
    ...
    T,
    E,
    FutureFn: AsyncFnMut() -> Result<T, E> + 'a,
...
impl<'a, B, T, E, FutureFn> Retry<'a, B, T, E, FutureFn>
where
    B: Backoff,
    FutureFn: AsyncFnMut() -> Result<T, E>,
{
...
impl<'a, B, T, E, FutureFn, SF, RF, NF> Retry<'a, B, T, E, FutureFn, SF, RF, NF>
where
    B: Backoff,
    FutureFn: AsyncFnMut() -> Result<T, E>,
    SF: MaybeSleeper,
    ...
    NF: FnMut(&E, Duration),
{
    ...
    /// }
    /// ```
    pub fn sleep<SN: Sleeper>(self, sleep_fn: SN) -> Retry<'a, B, T, E, FutureFn, SN, RF, NF> {
        Retry {
    ...
    pub fn when<RN: FnMut(&E) -> bool>(
        self,
        retryable: RN,
    ) -> Retry<'a, B, T, E, FutureFn, SF, RN, NF> {
        Retry {
            backoff: self.backoff,
            retryable,
            notify: self.notify,
    ...
    pub fn notify<NN: FnMut(&E, Duration)>(
        self,
        notify: NN,
    ) -> Retry<'a, B, T, E, FutureFn, SF, RF, NN> {
        Retry {
            backoff: self.backoff,
            retryable: self.retryable,
            notify,
...
impl<'a, B, T, E, FutureFn, SF, RF, NF> Future for Retry<'a, B, T, E, FutureFn, SF, RF, NF>
where
    B: Backoff,
    FutureFn: AsyncFnMut() -> Result<T, E> + 'a,
    SF: Sleeper,
...
mod default_sleeper_tests {
    ...
    #[test]
    async fn test_async_closure_retry() {
    ...
    #[test]
    async fn test_async_closure_retry_with_retryable_error() {
        let error_times = Mutex::new(0);
    ...
    #[test]
    async fn test_async_closure_fn_mut_when_and_notify() {
...
mod custom_sleeper_tests {
    ...
    #[test]
    async fn test_async_closure_retry_with_sleep() {
        let result = always_error
            .retry(ExponentialBuilder::default().with_min_delay(Duration::from_millis(1)))
            .sleep(|_| ready(()))
            .await;
